...
export default function OrderCard(props) {
  ...
  return (
    <>
        <Link href={{pathname: "/dashboard/orderDetails", query: {...order, ...user, ...orderDetails}}} className='linkTag'>
            <div className="card1">
                ...
                {order.status !== "cancelled" && <div className='orderDetails'>
                    <div>By {dueDate}</div>
        ...
        <div className="card2">
            <CancelBtn cancelHandler={cancelHandler} id={props.order._id} status={props.order.status}/>
            <DownloadBtn status={props.order.status}/>
        </div>
    </>
  )
}
